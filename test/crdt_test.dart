...
void main() {
  ...
  group('Seed', () {
    ...
    setUp(() {
      crdt = Crdt(MapStore({'x': Record(HybridLogicalClock(), 1)}));
    });
  ...
  group('Merge', () {
    ...
    test('Merge older', () async {
      await crdt.put('x', 2);
      await crdt.merge({'x': Record(HybridLogicalClock(now - 10000), 1)});
      var record = await crdt.get('x');
      expect(record.value, 2);
    });
    ...
    test('Merge very old', () async {
      await crdt.put('x', 2);
      await crdt.merge({'x': Record(HybridLogicalClock(now - 1000000), 1)});
      var record = await crdt.get('x');
      expect(record.value, 2);
    });
    ...
    test('Merge newer', () async {
      await crdt.put('x', 1);
      await crdt.merge({'x': Record(HybridLogicalClock(now + 1000000), 2)});
      var record = await crdt.get('x');
      expect(record.value, 2);
    });
    ...
    test('Merge same', () async {
      await crdt.put('x', 2);
      var remoteTs = (await crdt.get('x')).hybridLogicalClock;
    ...
    test('Merge older, newer counter', () async {
      await crdt.put('x', 2);
      await crdt.merge({'x': Record(HybridLogicalClock(now - 1000000, 2), 1)});
      var record = await crdt.get('x');
      expect(record.value, 2);
    });
    ...
    test('Merge same, newer counter', () async {
      await crdt.put('x', 1);
      var remoteTs = HybridLogicalClock(
          (await crdt.get('x')).hybridLogicalClock.micros, 2);
    ...
    test('Merge new item', () async {
      var map = {'x': Record<int>(HybridLogicalClock(), 2)};
    ...
    test('Merge deleted item', () async {
      await crdt.put('x', 1);
      await crdt.merge({'x': Record(HybridLogicalClock(now + 1000000), null)});
      var record = await crdt.get('x');
      expect(record.isDeleted, isTrue);
    });
  });
  ...
  group('Serialization', () {
    ...
    setUp(() {
      crdt = Crdt(
          MapStore({'x': Record<int>(HybridLogicalClock(1579633503110), 1)}));
    });

    test('To map', () async {
      expect(await crdt.getMap(),
          {'x': Record<int>(HybridLogicalClock(1579633503110), 1)});
    });

    test('jsonEncodeStringKey', () async {
      expect(jsonEncode(await crdt.getMap()),
          '{"x":{"hybridLogicalClock":"1970-01-19T06:47:13.476Z-0000","value":1}}');
    });

    test('jsonEncodeIntKey', () async {
      expect(
          crdtMap2Json({
            1: Record(HybridLogicalClock.fromLogicalTime(1579633475584), 1)
          }),
          '{"1":{"hybridLogicalClock":"1970-01-19T06:47:13.476Z-0000","value":1}}');
    });
  ...
  group('Custom class serialization', () {
    ...
    setUp(() {
      crdt = Crdt(MapStore({
        'x': Record<TestClass>(
            HybridLogicalClock(1579633503110), TestClass('test'))
      }));
    });
    ...
    test('jsonEncode', () async {
      expect(jsonEncode(await crdt.getMap()),
          '{"x":{"hybridLogicalClock":"1970-01-19T06:47:13.476Z-0000","value":{"test":"test"}}}');
    });
